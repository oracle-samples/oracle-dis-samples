...
class ParameterBuild:
  ...
  def build(self):
      ...
      if (self.entityName  is not None):
        ...
        entitystr= "dataref:"+self.connectionKey+"/"+self.schema+"/"+self.entityType+":"+self.entityName
        ee = None
        de = None
        if (self.entityType == "FILE_ENTITY"):
          ...
          key=entitystr,object_status=1,entity_type=DataEntityFromTable.ENTITY_TYPE_TABLE
          ,name=self.entityName,resource_name=self.entityName)

        if (self.dataFormat is not None):
          ee = EnrichedEntity(entity=de, data_format=self.dataFormat, model_type="ENRICHED_ENTITY")
        else:
        ...
        sch = Schema(key=schemakey,object_status=1,name=self.schema,resource_name=self.schema)
  ...
  def buildValue(self,opName):
      ...
      if (self.entityName  is not None):
        ...
        ee = None
        de = None
        shape = Shape(key=self.connectionKey+"/"+self.schema+"/"+self.entityType+":"+self.entityName+"/SHAPE", type=CompositeType())
        if (self.entityType == "FILE_ENTITY"):
          de = DataEntityFromFile(
          key=entitystr,
          object_status=8,
          ...
          de = DataEntityFromTable(
          key=entitystr,object_status=8,entity_type=DataEntityFromTable.ENTITY_TYPE_TABLE
          ,name=self.entityName,resource_name=self.entityType+":"+self.entityName)

        parentRef=ParentReference(parent=key)
        if (self.dataFormat is not None):
          ee = {"entity":de, "dataFormat":self.dataFormat, "modelType":"ENRICHED_ENTITY", "parentRef":parentRef}
        else:
          ee = {"entity":de, "modelType":"ENRICHED_ENTITY", "parentRef":parentRef}
        return ee
      return None
