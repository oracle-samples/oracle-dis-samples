...
def create_schedule(data_integration_client, workspace_id, application_key, schedule_name, timezone, cron_expression, daylight_enabled):
  create_schedule_response=data_integration_client.create_schedule(
    ...
    create_schedule_details=oci.data_integration.models.CreateScheduleDetails(
        ...
        frequency_details =oci.data_integration.models.CustomFrequencyDetails(custom_expression=cron_expression)
  ))
...
def create_task_schedule(data_integration_client, workspace_id, application_key,task_key,task_schedule_name,schedule_key,enabled):
    create_task_schedule_response=data_integration_client.create_task_schedule(
       ...
       create_task_schedule_details=oci.data_integration.models.CreateTaskScheduleDetails(
           ...
           schedule_ref=oci.data_integration.models.Schedule( object_status=1, key=schedule_key),
           is_enabled=enabled,
...
def create_task_schedules(data_integration_client, filename, workspace_id, application_key):
 with open(filename, newline='') as csvfile:
    ...
    for row in taskschreader:
        ...
        enabled = row[2]
        task_name = row[3]
        print("Creating task schedule " + task_schedule_name + " for task " + task_name + " using schedule " + schedule_name)
        sch=get_schedule_key(data_integration_client, workspace_id, application_key, schedule_name)
        tsk=get_task_key(data_integration_client, workspace_id, application_key, task_name)
        tasksch=create_task_schedule(data_integration_client, workspace_id, application_key,tsk.key,task_schedule_name,sch.key,bool(enabled))
...
def create_schedules(data_integration_client, filename, workspace_id, application_key):
 with open(filename, newline='') as csvfile:
    ...
    for row in schedulereader:
        ...
        timezone = row[2]
        enabled = row[3]
        print("Creating schedule " + schedule_name + " using cron expression " + cron_expression + " in timezone " + timezone)
        sch=create_schedule(data_integration_client, workspace_id, application_key,schedule_name,timezone, cron_expression, bool(enabled))
        print(sch.data.key)
...
try:
  ...
  for currentArgument, currentValue in arguments:
    if currentArgument in ("-h", "--Help"):
      print ("--workspace workspace_id --application application_key -inputfile filename")
...
if (object_type == "TASK_SCHEDULE"):
 create_task_schedules(data_integration_client, filename, workspace_id, application_key)
else:
 create_schedules(data_integration_client, filename, workspace_id, application_key)
